...
class Detect(metaclass=logMeta(logger)):
    ...
    def getFieldServant(self):
        ...
    def getHP(self):return[self._ocr((200+318*i,620,293+318*i,644))for i in range(3)]
    def getNP(self):return[self._ocr((220+318*i,655,274+318*i,680))for i in range(3)]
    ...
    def getTeamServant(self):return[
        ...
        )[1])((lambda img:[cv2.resize(img,(size,size))for size in range(20,65,4)])(self._crop((47+200*pos+15*(pos>2),224,220+200*pos+15*(pos>2),370))))
        for pos,cls,card in tqdm.tqdm(list(zip(range(6),self.getTeamServantClass(),self.getTeamServantCard())))
    ]
