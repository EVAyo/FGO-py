...
更改:分辨率变为1280*720  
原先的数字OCR被压烂了,现在会丢数字  
新增:getTeamServant系列识别,在编队界面获取编队中的各个角色的从者序号  
...
新增:gui和web下的bench  
同时cli下的bench现有可选的参数  
另:前阵子加回了win32的截图,使我不由得回顾了本项目的发展历程  
...
新增(?):win32  
我看BBChannel新增的让输入如输出使用不同调试方式的所谓「混合连接」理念非常不错,不同的模拟器有不同的窗口行为,并不见得同时存在又能点击又能截图的窗口,于是加回了v2.8.0版本的win32截图并附加了鼠标操作  
...
新增:AidLux提供了现成的python3.9,对应修改了AdiLux.sh  
...
优化:[pr #43](https://github.com/hgjazhgj/FGO-py/pull/43)  
...
新增:技能后点击屏幕  
这个功能国服还没有上线,但是本次游戏更新确实有一些未在公告中展示的新增内容(也可能是我搞错了),所以这样前向兼容的小功能就先写了  

## 2022/01/06 v7.10.1

更改:调整了三色指令卡的模板以应对新版游戏ui更改  
...
新增:BlueStacks 5 with hyper-V的快速连接支持  
...
优化:Check截图的文件名改为截图的时间而非保存的时间  
新增:为所有技能添加了优先级,优先级数值越大,对应技能越先被施放  
这个优先级是打通了从者技能和御主技能的  
在优先级相同时,*在ui中写在前面的先放*  
更改:每放一个技能就会刷新所有技能的冷却状态  
...
新增:修改了基础的图像识别算法,模板同时被用作蒙版,如果一个像素三通道都不大于1就会被忽略  
这个更改可能导致目前已经存在的识别参数需要些许调整  
这个更改允许你制作非矩形/非连通的助战模板,参考fgoImage/friend下新增的图片  
新增:选卡时考虑暴击  
新活动的逆克制加成有点顶,所以又优化了一波战斗算法  
这里其实拿边缘去匹配效果会好得多,但是懒  
优化:缩小了一些图片模板,大幅节约你的硬盘  
优化(?):调整了指令卡分组的阈值,具体情况有待进一步观察  
优化:降低了红卡/color chain/extra attack的权重  
...
新增:specialDrop除了新物品以外还可以是fgoImage/dropFilter下的png  
...
新增:添加托盘图标的「退出」和「强制退出」菜单,移除主窗口的「退出」菜单选项,以及其他一些ui上的小优化  
...
新增:智能选择每面血量最高的敌人,弃用「优先攻击」设定  
新增:增加了一些可保存的配置项,置于`fgoConfig.json`内  
新增:添加托盘图标,可设置关闭到托盘  
在启用「关闭到托盘」时,通过「文件」菜单中的「退出」来关闭进程  
新增:战斗完成时通知,取代原先的beep  
系统通知受勿扰与专注等系统设置控制  
...
新增:手写了一个轻量级的ocr,以识别我方的HP/NP和敌方的HP  
...
新增:刚出了c呆羁绊暂时还没满,就把换人UI加回来了,并添加了相关代码  
...
这破游戏也五周年了...  
bugfix:[issue #21](https://github.com/hgjazhgj/FGO-py/issues/21)  
新增:指令卡分组,可以打出Brave Chain  
...
新增:选卡考虑克制关系,以应对多职阶混合本,或是提高速刷后补刀效率  
...
新增:重构部分代码,使之具有能在错误发生时不丢失当前战斗状态的特性  
...
注意:本更新的兼容性未经充分验证  
新增:试图获取当前连接是否仍然可用,以替代先前的是否曾经建立过连接,并在从暂停中恢复程序执行时添加判断  
...
新增:既然又要动到具体实现了,我就把我自己的swipe改了改加回去了  
新增:在屏幕方向发生改变时智能更新,移除了手动更新选项  

## 2021/07/06 v6.2.0

新增:适配airtest 1.2.0,具体参见[https://github.com/AirtestProject/Airtest/releases/tag/v1.2.0](https://github.com/AirtestProject/Airtest/releases/tag/v1.2.0)  
...
新增:移除了我自己编写的swipe  
新增:对于游戏画面不在屏幕中央的设备,在屏幕旋转后可以在「控制」菜单中刷新方向而无需重新建立连接  
...
新增:更改了连接设备的UI,现在你可以直接连接到远程设备,只需要在原先的选择设备对话框里输入对应地址  
同时,现在连接上的设备会显示出来  
更改:不WrapLongRows  
你uic有bug我就不用咯  
另:我发现airtest有严重的内存泄漏与僵尸线程问题,请尽量不要异常断开连接或是切换设备,如果你发现本项目异常占用系统资源,目前只能关闭整个进程重开  
...
更改:移除了PowershellHere  
bug修复:将上次遗漏的各Flag由enum改为enum class  
...
新增:增加requirements.txt,修订readme  
(更新||bug修复)&&优化:延时调整  
...
更改:删除对airtest.core.android.adb.ADB.getPhysicalDisplayInfo相关的bugfix  
参见v4.4.0日志  
...
bug修复:在程序执行时获取截图有可能冲突挺烦人的,就手动写了个锁,反正单例模式没必要调库,到底没几行代码,方便简单透明高效  
...
新增:最近长草,有一些幕间本/强话本少于3面,增加了一些代码让技能和宝具能够提前使用,让你不必频繁更改队伍配置  
...
新增:基本功能基本稳定,多设备兼容性也没有问题,但仍有待优化  
...
新增:我已经基本搞明白了airtest安卓部分的工作原理,现在已经用上了airtest库进行截图与触摸,效率奇高  
但是现在点得太快了,又得手动延迟  
...
bug修复:现在当hFgoWnd最小化时启动程序不会把程序窗口移到屏幕外面去  
...
即将新增:编队设置的新建/重命名/删除等功能.代码已经写好了,但是效果有点不友好,所以相关控件被改为了不可编辑不可点击  
...
优化:现在这个程序可以用于任意分辨率的16:9的模拟器显示画面了  
